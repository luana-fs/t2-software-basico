...
mymemory_t *mymemory_init(size_t size)
{
    ...
    if (!manager)
    {                // troquei a logica do if só por conveniencia - raffa
    ...
    if (!manager->pool)
    {                  // tambem troquei a logica para o uso de not por conveniencia - raffa
...
void mymemory_stats(mymemory_t *memory)
{
    ...
    // Endereco imediatamente após o final do pool
...
{
    if (!memory)
        return; // caso não tenha nada em memoria ele simplesmente não executa o método
